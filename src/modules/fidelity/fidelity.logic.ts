...
export class FidelityLogic {
    ...
    async recalculateUserLevel(userId: string): Promise<LevelResult> {
            ...
            switch (conditions.type) {
                case "COUNT":
                    if (conditions.action_key === actionKey) {
                        const field = this.getStatFieldForAction(conditions.action_key);
                        if (field && userStats) {
    ...
    async getFidelitySummary(userId: string): Promise<FidelitySummary> {
        const stats = await fidelityRepository.getUserStats(userId);
    ...
    async getUserBadges(userId: string): Promise<UserBadgesView> {
        const allBadges = await fidelityRepository.getAllBadges();
        const userBadges = await fidelityRepository.getUserBadges(userId);
        const userStats = await fidelityRepository.getUserStats(userId);
    ...
    async getUserChallenges(userId: string): Promise<UserChallengesView> {
        const userChallenges = await fidelityRepository.getUserChallenges(userId);
    ...
    async handleFidelityEvent(event: FidelityEvent): Promise<FidelityEventResult> {
        const { type, userId, payload } = event;

        const actionKey = this.mapEventToAction(type);
        if (!actionKey) {
            return {
                pointsAwarded: 0,
                badgesUnlocked: [],
        ...
        await this.updateUserStatsForAction(userId, actionKey, payload);

        const levelResult = await this.recalculateUserLevel(userId);

        const badgesUnlocked = await this.checkAndUnlockBadges(userId, actionKey, payload);

        const challengesUpdated = await this.updateChallenges(userId, actionKey, payload);
    ...
    private mapEventToAction(eventType: string): FidelityActionKey | null {
    ...
    private getStatFieldForAction(actionKey: string): string | null {
    ...
    async getAllLevels() {
        return await fidelityRepository.getAllLevels();
    }
