...
class SubscriptionsController {
    ...
    readonly createCheckout = this.factory.createHandlers(
        ...
        async (ctx) => {
            ...
            try {
                ...
                if (!stripeCustomerId) {
                    // Get user email
                    const userResult = await this.userRepository.getUserById(
                        user.id,
                    );
                    ...
                    await subscriptionsRepository.setStripeCustomerId(
                        user.id,
                        stripeCustomerId,
                    );
                }
    ...
    readonly getMySubscription = this.factory.createHandlers(async (ctx) => {
        ...
        try {
            const subscription =
                await subscriptionsRepository.getSubscriptionByUserId(user.id);
    ...
    readonly updatePaymentMethod = this.factory.createHandlers(async (ctx) => {
        ...
        try {
            const stripeCustomerId =
                await subscriptionsRepository.getStripeCustomerId(user.id);
            ...
            const portalSession = await stripe.billingPortal.sessions.create({
                customer: stripeCustomerId,
                return_url: CHECKOUT_URLS.SUCCESS.replace(
                    "?checkout=success",
                    "",
                ),
            });
    ...
    readonly upgradeSubscription = this.factory.createHandlers(
        ...
        async (ctx) => {
            ...
            try {
                const subscription =
                    await subscriptionsRepository.getSubscriptionByUserId(
                        user.id,
                    );
    ...
    readonly getMyInvoices = this.factory.createHandlers(async (ctx) => {
        ...
        try {
            const invoices = await subscriptionsRepository.getInvoicesByUserId(
                user.id,
            );
