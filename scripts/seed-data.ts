...
import {
    ...
    teamsTable,
    type Team,
} from "../src/config/db/sports.table";
...
const defaultOpeningHours: OpeningHours = [
    ...
    },
    {
        day_of_week: 4,
        is_closed: false,
        periods: [{ open: "11:00", close: "00:00" }],
    },
    {
        day_of_week: 5,
        is_closed: false,
        periods: [{ open: "11:00", close: "01:00" }],
    },
    {
        day_of_week: 6,
        is_closed: false,
        periods: [{ open: "12:00", close: "01:00" }],
    },
];
...
async function seedUsers(): Promise<string[]> {
    ...
    if (admin) {
        ...
        const [existingAdmin] = await db
            ...
            .from(usersTable)
            .where(eq(usersTable.email, "admin@matchapp.com"));
        if (existingAdmin) userIds.push(existingAdmin.id);
    }
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (owner) {
            ...
            if (existingOwner) userIds.push(existingOwner.id);
        }
    }
...
async function seedSubscriptionsAndVenues(
    ownerIds: string[],
): Promise<{ subscriptionIds: string[]; venueIds: string[] }> {
    ...
    for (let i = 0; i < ownerIds.length; i++) {
        ...
        const [subscription] = await db
            ...
            .values({
                ...
                max_monthly_reservations:
                    plan === "basic" ? 100 : plan === "pro" ? 500 : 9999,
        ...
        if (!subscription) {
            // Fetch existing subscription for this owner
            const [existingSub] = await db
                .select({ id: subscriptionsTable.id })
                .from(subscriptionsTable)
                .where(eq(subscriptionsTable.user_id, ownerId));
            if (!existingSub) continue;
            subscriptionId = existingSub.id;
            subscriptionIds.push(existingSub.id);

            // Also check for existing venue
        ...
        if (!venue) {
            // Fetch existing venue for this owner
            const [existingVenue] = await db
                .select({ id: venuesTable.id })
                .from(venuesTable)
                .where(eq(venuesTable.owner_id, ownerId));
            ...
            await db
                ...
                .values({
                    ...
                    name:
                        t < 5
                            ? `Table ${t + 1}`
                            : `Booth ${String.fromCharCode(65 + t - 5)}`,
...
async function seedMatches(): Promise<string[]> {
    ...
    for (const league of leagues) {
        const teams = await db
            .select()
            .from(teamsTable)
            .where(eq(teamsTable.league_id, league.id));
...
async function seedVenueMatches(
    venueIds: string[],
    matchIds: string[],
): Promise<string[]> {
    ...
    for (const venueId of venueIds) {
        const selectedMatches = randomElements(
            matchIds,
            randomInt(3, Math.min(8, matchIds.length)),
        );

        for (const matchId of selectedMatches) {
            const totalCapacity = randomInt(30, 100);
            const reservedCapacity = randomInt(
                0,
                Math.floor(totalCapacity * 0.6),
            );

            const [venueMatch] = await db
                .insert(venueMatchesTable)
                .values({
                    venue_id: venueId,
                    match_id: matchId,
                    ...
                    show_on_map: true,
                    estimated_crowd_level: randomElement([
                        "low",
                        "medium",
                        "high",
                    ]),
...
async function seedReservations(
    userIds: string[],
    venueMatchIds: string[],
): Promise<void> {
    console.log(
        "\n🎫 Seeding reservations (including many recent ones for last 30 days)...",
    );
    ...
    for (const userId of regularUsers) {
        ...
        const selectedVenueMatches = randomElements(
            venueMatchIds,
            numReservations,
        );
        ...
        for (const venueMatchId of selectedVenueMatches) {
            ...
            await db
                ...
                .values({
                    ...
                    quantity: partySize,
                    special_requests:
                        Math.random() > 0.7 ? "Window seat preferred" : null,
                    checked_in_at:
                        status === "completed"
                            ? generatePastDate(randomInt(1, 7))
                            : null,
                    completed_at:
                        status === "completed"
                            ? generatePastDate(randomInt(1, 7))
                    ...
                    canceled_at:
                        status === "canceled"
                            ...
                            : null,
                    canceled_reason:
                        status === "canceled" ? "Plans changed" : null,
                })
                .onConflictDoNothing();
    ...
    // Create MANY more recent reservations (last 30 days) for realistic dashboard stats
    console.log("  📅 Adding extra recent reservations for last 30 days...");
    const recentStatuses = ["confirmed", "completed"] as const; // More realistic recent statuses

    for (let day = 0; day < 30; day++) {
        // Create 5-15 reservations per day for last 30 days
        const reservationsPerDay = randomInt(5, 15);

        for (let r = 0; r < reservationsPerDay; r++) {
            const userId = randomElement(regularUsers);
            const venueMatchId = randomElement(venueMatchIds);
            const status = randomElement(recentStatuses);
            const partySize = randomInt(1, 8);
            const createdAt = generatePastDate(day);
            ...
            await db
                ...
                .values({
                    ...
                    special_requests:
                        Math.random() > 0.8
                            ? randomElement([
                                  "Near the big screen please",
                                  "Birthday celebration!",
                                  "Quiet corner if possible",
                                  "Window seat preferred",
                                  "Need high chairs for kids",
                              ])
                            : null,
                    checked_in_at: status === "completed" ? createdAt : null,
    ...
    console.log(`  📈 Recent reservations (last 30 days): ~${30 * 10} extra`);
}
...
async function seedNotifications(userIds: string[]): Promise<void> {
    ...
    const messages: Record<NotificationType, string> = {
        reservation_confirmed:
            "Your reservation has been confirmed! See you at the match.",
        reservation_reminder:
            "Reminder: Your reservation is coming up in 2 hours.",
        ...
        for (let n = 0; n < numNotifications; n++) {
            ...
            await db
                ...
                .values({
                    ...
                    read_at:
                        Math.random() > 0.4
                            ? generatePastDate(randomInt(0, 7))
                            : null,
...
async function seedUserFavorites(
    ...
    for (const userId of regularUsers) {
        ...
        for (const venueId of selectedVenues) {
            await db
                .insert(userFavoriteVenuesTable)
                .values({
                    user_id: userId,
                    venue_id: venueId,
                })
